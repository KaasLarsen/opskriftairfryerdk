---
title: 'Airfryer BBQ kyllingelår – sticky glaze og sprød kant'
description: 'BBQ kyllingelår i airfryer: først krydret steg ved 190 °C, BBQ-sauce til sidst cirka 30–34 min i alt til mindst **75 °C** — sticky glaze uden brændt sukker.'
heroImage: '/images/recipes/airfryer-bbq-kyllingelaar.png'
heroAlt: 'BBQ-glaserede kyllingelår med blank overflade, dip og citron på tallerken foran en airfryer'
pubDate: 2026-03-31
updatedDate: 2026-05-21
prepMinutes: 10
cookMinutes: 32
servings: '4 portioner'
calories: 'Ca. 450 kcal pr. portion'
category: 'Hovedret'
cuisine: 'Amerikansk'
temperatureC: 190
keywords:
  - bbq kyllingelår airfryer
  - sticky kylling airfryer
  - airfryer kyllingelår marinade
  - kyllingelår bbq
  - nem weekendmad
  - bbq kylling air fry tid
featured: true
intro: |
  BBQ kyllingelår i airfryer er en stærk variant af den klassiske kyllingelårsopskrift. Du får saftigt kød, sprød kant og en klæbrig glaze uden at tænde grillen.

  For at undgå brændt sukker lægger vi BBQ-saucen på sent i processen. Så når saucen at karamellisere uden at blive bitter.

  Opskriften er perfekt til weekendmad, gæster eller når du vil have lidt ekstra smag i hverdagen.
conclusion: |
  Server kyllingelårene varme med ekstra BBQ-sauce ved siden af. Frisk salat eller syrligt tilbehør balancerer sødmen godt.

  Lav gerne en stor portion; rester er gode i wraps eller burgerboller dagen efter.

  Tilbehør som [airfryer pomfritter](/opskrifter/airfryer-pomfritter) eller [airfryer søde kartofler](/opskrifter/airfryer-soede-kartofler) passer perfekt.
ingredients:
  - text: '4-6 kyllingelår'
  - text: '1 spsk olie'
  - text: '1 tsk salt'
  - text: '1/2 tsk peber'
  - text: '1 tsk paprika'
  - text: '1/2 tsk hvidløgspulver'
  - text: '5-6 spsk BBQ-sauce'
instructions:
  - text: 'Dup kyllingelårene tørre og vend med olie og tørre krydderier.'
  - text: 'Forvarm airfryeren til 190 °C.'
  - text: 'Steg kyllingelårene 18 minutter.'
  - text: 'Vend lårene og steg 8 minutter mere.'
  - text: 'Pensl med BBQ-sauce og steg 4-6 minutter, til glaze er blank og let karamelliseret.'
  - text: 'Tjek kernetemperatur til mindst 75 °C.'
tips:
  - 'Læg sauce på sent for at undgå brankning.'
  - 'Brug gerne to lag sauce i de sidste minutter for ekstra sticky overflade.'
  - 'Sænk temperaturen lidt hvis saucen bliver mørk for hurtigt.'
faq:
  - question: 'Hvornår skal BBQ-sauce på i airfryer?'
    answer: 'Til sidst i stegningen, så sukkeret i saucen ikke brænder.'
  - question: 'Hvor længe skal BBQ kyllingelår have i airfryer?'
    answer: 'Typisk 30-34 minutter totalt ved 190 °C, inkl. glaze-fase.'
  - question: 'Kan jeg bruge færdig BBQ-sauce?'
    answer: 'Ja, færdig sauce fungerer fint. Vælg en med smag du kan lide, og justér salt efter behov.'
  - question: 'Hvilken kernetemperatur er sikkert på kyllingelår?'
    answer: 'Mindst **75 °C** tættest på benet eller i den tykkeste del — mål også efter glaze, da sukker giver anderledes overflade.'
---

### BBQ-glaze uden gril

Sød og røget BBQ-sauce giver grillstemning, selv når du bruger airfryer – vigtigste trick er at tilføje den sent, så sukker ikke brænder fast.

### Sådan får du balance i BBQ-smagen

Mange BBQ-saucer er søde. For en bedre balance kan du tilsætte lidt syre i form af æblecidereddike eller citron ved servering. Det fremhæver kødet og gør retten mindre tung, især hvis du serverer den med kartofler eller brød.

### Ekstra idé til servering

Lav en hurtig coleslaw med kål, gulerod og en let dressing ved siden af. Den sprøde, friske side passer virkelig godt til den karamelliserede overflade på kyllingelårene og giver en mere komplet tallerken.
