---
title: 'Airfryer broccoli – sprød og grøn på 12 minutter'
description: 'Broccoli i airfryer med let sprød kant og saftig bid. Nem tilbehørsopskrift med tid, temperatur og krydring.'
heroImage: '/images/recipes/airfryer-broccoli.png'
heroAlt: 'Sprødstegt broccoli med dip på tallerken foran en airfryer'
pubDate: 2026-03-31
updatedDate: 2026-03-31
prepMinutes: 6
cookMinutes: 12
servings: '4 portioner'
calories: 'Ca. 120 kcal pr. portion'
category: 'Tilbehør'
cuisine: 'International'
temperatureC: 190
keywords:
  - broccoli i airfryer
  - airfryer broccoli
  - grøntsager i airfryer
  - sundt tilbehør
  - nem broccoli opskrift
featured: true
intro: |
  Broccoli i airfryer er et hurtigt grønt tilbehør, der passer til næsten alt. Du får mere smag end kogt broccoli, fordi kanterne karamelliserer let.

  Hemmeligheden er ens buketter, en smule olie og ikke for høj fyldning i kurven.

  Opskriften her kan varieres med citron, chili eller parmesan.
conclusion: |
  Server broccoli med det samme for bedste tekstur. Brug evt. citronsaft og lidt flagesalt ved servering.

  Broccoli passer til både fisk, kød og vegetarretter.

  Prøv sammen med [airfryer laks](/opskrifter/airfryer-laks) eller [airfryer koteletter](/opskrifter/airfryer-koteletter).
ingredients:
  - text: '1 stort broccoli-hoved (ca. 500 g)'
  - text: '1 spsk olie'
  - text: '1/2 tsk salt'
  - text: '1/4 tsk peber'
  - text: '1 fed hvidløg (valgfrit, finthakket)'
instructions:
  - text: 'Del broccoli i ens små buketter og tør dem let.'
  - text: 'Vend med olie, salt, peber og evt. hvidløg.'
  - text: 'Forvarm airfryeren til 190 °C.'
  - text: 'Steg broccoli 10-12 minutter og ryst kurven halvvejs.'
tips:
  - 'Mindre buketter bliver hurtigt mørke; hold øje mod slutningen.'
  - 'Tilføj citronskal for frisk smag.'
  - 'Top med parmesan de sidste 2 minutter for ekstra umami.'
faq:
  - question: 'Hvor længe skal broccoli have i airfryer?'
    answer: 'Typisk 10-12 minutter ved 190 °C.'
  - question: 'Kan broccoli blive sprød i airfryer?'
    answer: 'Ja, især i kanterne, hvis buketterne ligger i ét lag.'
  - question: 'Kan jeg bruge frossen broccoli?'
    answer: 'Ja, men den bliver ofte blødere. Tø og tør den gerne først.'
---

### Hurtigt grønt til hverdag

Broccoli i airfryer giver mere smagsdybde end kogt broccoli og er et af de nemmeste grøntsager tilbehør at kombinere med både fisk, kød og vegetar.

### Undgå blød broccoli

Våd broccoli damper i kurven og mister den lette sprødhed. Sørg derfor for at tørre buketterne efter skylning, og brug kun en lille mængde olie. Et enkelt lag i kurven gør en større forskel end de fleste tror.

### Nem variation med ekstra smag

Prøv at afslutte med revet citronskal, chiliflager eller ristede sesamfrø. Små toppings gør broccoli mere interessant og kan få grøntsager til at glide lettere ned hos både børn og voksne.
