---
title: 'Airfryer søde kartofler – sprøde fries og både'
description: 'Søde kartofler i airfryer med karamelliseret kant og blød midte. Nem opskrift med tid, temperatur og krydderiforslag.'
heroImage: '/images/recipes/airfryer-soede-kartofler.png'
heroAlt: 'Sprøde søde kartoffelfritter med cremet dip på tallerken foran en airfryer'
pubDate: 2026-03-31
updatedDate: 2026-03-31
prepMinutes: 10
cookMinutes: 18
servings: '4 portioner'
calories: 'Ca. 230 kcal pr. portion'
category: 'Tilbehør'
cuisine: 'International'
temperatureC: 200
keywords:
  - søde kartofler airfryer
  - airfryer sweet potato fries
  - airfryer søde kartoffel fritter
  - sød kartoffel i airfryer
  - sundt tilbehør airfryer
featured: true
intro: |
  Søde kartofler i airfryer er et populært alternativ til klassiske fritter. De får en naturlig sødme og let karamellisering, når de steges ved høj varme i et tyndt lag.

  Konsistensen bliver bedst, hvis stykkerne er ens i størrelse, og du undgår overfyldning. Søde kartofler bliver sjældent helt så "tør-sprøde" som almindelige pommes, men kan blive rigtig lækre med korrekt teknik.

  Denne opskrift fungerer både som fries og grove både.
conclusion: |
  Server med det samme for bedste tekstur. Kombinér med dip som hvidløgs-yoghurt, chili-mayo eller lime-creme.

  Søde kartofler passer godt til både fisk, kylling og vegetarretter.

  Prøv dem som side til [airfryer fiskefilet](/opskrifter/airfryer-fiskefilet) eller [airfryer kyllingespyd](/opskrifter/airfryer-kyllingespyd).
ingredients:
  - text: '900 g søde kartofler'
  - text: '1 1/2 spsk olie'
  - text: '1 tsk salt'
  - text: '1/2 tsk paprika'
  - text: '1/4 tsk hvidløgspulver (valgfrit)'
instructions:
  - text: 'Skræl de søde kartofler og skær i ens stave eller både.'
  - text: 'Vend dem med olie, salt og krydderier.'
  - text: 'Forvarm airfryeren til 200 °C.'
  - text: 'Steg i ét lag i 10 minutter.'
  - text: 'Ryst kurven og steg yderligere 7-9 minutter, til gylden kant og mør midte.'
tips:
  - 'Steg i to hold hvis mængden er stor.'
  - 'Tilsæt evt. majsstivelse (1 tsk) for mere overfladesprødhed.'
  - 'Hold øje de sidste minutter, da sukkerindholdet kan give hurtig farvning.'
faq:
  - question: 'Hvor længe skal søde kartofler have i airfryer?'
    answer: 'Typisk 17-20 minutter ved 200 °C afhængigt af tykkelse og model.'
  - question: 'Hvorfor bliver søde kartofler bløde i airfryer?'
    answer: 'For stor mængde i kurven og for lidt varmecirkulation giver damp og blød overflade.'
  - question: 'Skal søde kartofler udvandes først?'
    answer: 'Det er ikke nødvendigt som ved almindelige kartofler, men du kan teste kort udvanding hvis du ønsker.'
---

### Populær long-tail opskrift

Søgeord om søde kartofler i airfryer er stabile og matcher sundere hverdags-intent.

### Sådan får du bedre overflade

Søde kartofler indeholder mere sukker og bliver derfor hurtigere mørke end almindelige kartofler. Hold øje i de sidste minutter, og steg i mindre hold for at undgå blød tekstur. Et lille drys stivelse kan give mere bid i kanten.

### Smagskombinationer

Søde kartofler passer godt med både stærke og syrlige dips. Prøv chipotle-mayo, lime-yoghurt eller en klassisk hvidløgsdip. Det gør dem til et alsidigt tilbehør til både fisk, kylling og vegetarretter.
