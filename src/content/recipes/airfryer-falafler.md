---
title: 'Airfryer falafler – sprøde uden friture'
description: 'Falafler i airfryer med sprød yderside og blød midte. Nem vegetarisk opskrift med tid, temperatur og tips.'
heroImage: '/images/recipes/airfryer-falafler.png'
heroAlt: 'Sprøde gyldne falafler med dip og friske grøntsager på tallerken foran en airfryer'
pubDate: 2026-03-31
updatedDate: 2026-03-31
prepMinutes: 20
cookMinutes: 14
servings: '4 portioner'
calories: 'Ca. 300 kcal pr. portion'
category: 'Vegetar'
cuisine: 'Mellemøstlig'
temperatureC: 195
keywords:
  - falafler i airfryer
  - airfryer falafler
  - vegetar airfryer opskrift
  - sprøde falafler
  - kikærter airfryer
featured: true
intro: |
  Falafler i airfryer er et godt alternativ til friture, når du vil have sprødhed med mindre olie. Med korrekt konsistens i farsen får du en flot overflade og saftig midte.

  Brug gerne tørrede kikærter, der er udblødt natten over, for bedste struktur. Konserves kan bruges, men kræver ekstra opmærksomhed på fugt.

  Opskriften her er vegetarisk og egner sig godt til pita, bowls eller salat.
conclusion: |
  Falafler er bedst helt varme og sprøde fra airfryer. Server med tahin-dressing, syltede løg og friske urter.

  Rester kan gemmes og varmes kort igen i airfryer, så de får sprødheden tilbage.

  Kombiner med en frisk salat eller bagte grøntsager for et komplet måltid.
ingredients:
  - text: '250 g tørrede kikærter (udblødt natten over)'
  - text: '1 lille løg'
  - text: '2 fed hvidløg'
  - text: '1 håndfuld persille'
  - text: '1 tsk spidskommen'
  - text: '1 tsk koriander'
  - text: '1 tsk salt'
  - text: '1/2 tsk peber'
  - text: '1 spsk kikærtemel eller hvedemel'
  - text: '1 spsk olie'
instructions:
  - text: 'Dræn de udblødte kikærter grundigt.'
  - text: 'Blend kikærter med løg, hvidløg, persille og krydderier til grov fars.'
  - text: 'Rør mel i og lad farsen hvile 10 minutter.'
  - text: 'Form små kugler eller flade deller og pensl let med olie.'
  - text: 'Forvarm airfryeren til 195 °C.'
  - text: 'Steg falaflerne 7 minutter, vend og steg yderligere 6-7 minutter.'
tips:
  - 'Er farsen for våd, tilsæt lidt ekstra mel.'
  - 'Steg i hold for bedst luftcirkulation.'
  - 'Flade falafler bliver ofte jævnere sprøde end helt runde.'
faq:
  - question: 'Kan man lave falafler i airfryer uden olie?'
    answer: 'Ja, men et tyndt lag olie giver markant bedre farve og sprødhed.'
  - question: 'Hvor længe skal falafler have i airfryer?'
    answer: 'Normalt 13-15 minutter ved 195 °C afhængigt af størrelse.'
  - question: 'Kan jeg bruge kikærter fra dåse?'
    answer: 'Ja, men resultatet bliver ofte blødere. Sørg for at dræne og tørre dem meget grundigt.'
---

### Stærk vegetar-trafik

Falafler er en populær vegetarisk søgning og supplerer jeres eksisterende kød/fisk-opskrifter godt.

### Form og størrelse betyder meget

Mindre falafler får mere overflade og bliver hurtigere sprøde. Hvis du ønsker en blødere midte, kan du lave dem lidt større og fladere. Test én første omgang, og justér form og tid til præcis den tekstur du foretrækker.

### Servering i hverdagen

Falafler er ideelle til pita med salat, tomat og dressing, men de fungerer også godt i en bowl med ris eller bulgur. Lav gerne en dobbelt portion, så du har hurtig vegetarprotein klar til næste dags frokost.
