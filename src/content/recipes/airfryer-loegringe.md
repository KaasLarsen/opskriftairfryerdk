---
title: 'Airfryer løgringe – sprøde uden friture'
description: 'Løgringe i airfryer med panko-panering: typisk cirka **13–15 min ved 190–200 °C**, vend én gang — sprød snack uden fritbad.'
heroImage: '/images/recipes/airfryer-loegringe.png'
heroAlt: 'Skål med gyldne panerede løgringe, ketchup og hvid dip, persille og airfryer med 200 grader i baggrunden'
pubDate: 2026-04-03
updatedDate: 2026-05-21
prepMinutes: 20
cookMinutes: 16
servings: '4 portioner som snack'
calories: 'Ca. 220 kcal pr. portion (afhænger af panering og olie)'
category: 'Tilbehør'
cuisine: 'International'
temperatureC: 200
keywords:
  - airfryer løgringe
  - løgringe i airfryer
  - onion rings airfryer
  - sprøde løgringe
  - løgringe air fry tid
  - snack airfryer
featured: true
intro: |
  Løgringe i airfryer kan blive **rigtig sprøde**, hvis du bruger **klassisk paneringsrækkefølge**, tørrer løgene godt og giver **lidt olie på overfladen**.

  Store, sødlige løg (fx gule) giver tykke ringe der er nemmere at håndtere end alt for tynde skiver.

  Passer som snack til [burgere](/opskrifter/airfryer-burger), [hotdogs](/opskrifter/airfryer-grillpoelser-hotdog) eller som tilbehør til [kyllingenuggets](/opskrifter/airfryer-kyllingenuggets).
conclusion: |
  Dyp i ketchup, ranch, chillimayo eller aioli – server straks for bedste sprødhed.

  Vil du have mere “fast food”-menu, kombinér med [mozzarella sticks](/opskrifter/airfryer-mozzarella-sticks) eller [jalapeno poppers](/opskrifter/airfryer-jalapeno-poppers).

  Anden omgang i airfryer genopvarmer ringene, men yderst lag bliver sjældent lige så sprødt som første gang.
ingredients:
  - text: '2 store gule løg'
  - text: '1 dl mel + 1/2 tsk salt + 1/4 tsk peber'
  - text: '2 æg, pisket med 1 spsk vand eller mælk'
  - text: '1 1/2 dl panko eller rasp (panko giver oftest mest crunch)'
  - text: '1/2 tsk paprika eller hvidløgspulver (valgfrit i rasp)'
  - text: 'Oliespray eller 1 spsk olie til fordeling på ringene'
instructions:
  - text: 'Skær løg i skiver på ca. 1 cm og skil forsigtigt til ringe – brug de to midterste lag og gem resten til andre retter.'
  - text: 'Dup ringene tørre med køkkenrulle – klistrede overflader giver klumpet panering.'
  - text: 'Vend først i krydret mel, derefter i æg, til sidst i panko/rasp. Tryk panering let fast.'
  - text: 'Læg på en tallerken og spray eller pensl meget let med olie på begge sider.'
  - text: 'Forvarm airfryeren til 200 °C (eller 190 °C hvis paneringen bruner for hurtigt).'
  - text: 'Fordel ringe i ét lag med mellemrum (steg i 2-3 omgange ved behov). Steg 7 minutter, vend forsigtigt med tang, steg 6-8 minutter mere til gyldne og sprøde – justér efter tykkelse.'
  - text: 'Salt let ved servering.'
tips:
  - 'Som på fotoet: server i skål med bagepapir, ketchup + cremet dip og lidt persille – gæster kan dyppe i begge saucer.'
  - 'For tynde ringe: kortere tid og hold øje så rasp ikke brænder.'
  - 'Våde ringe = blød panering. Tør grundigt.'
  - 'Kan fryses ubagte på bagepapir i lag – steg fra frost med 2-3 minutter ekstra.'
faq:
  - question: 'Hvorfor er mine løgringe bløde i airfryer?'
    answer: 'Oftest for meget fugt i løget, for tyk panering uden olie på overfladen, eller overfyldt kurv.'
  - question: 'Skal jeg blanchere løgringe før airfryer?'
    answer: 'Som regel nej til denne metode – blanchering kan gøre dem mere våde. Start med tørre rå ringe.'
  - question: 'Kan jeg bruge kun rasp uden panko?'
    answer: 'Ja – resultatet bliver ofte mindre luftigt, men stadig godt med nok olie og ens tykkelse.'
  - question: 'Fedtindhold?'
    answer: 'Langt mindre end friture; det meste fedt kommer fra æg og lidt olie til sprødhed.'
  - question: 'Hvor længe pr. omgang i “air fry”-maskine?'
    answer: 'Ofte **7 min + 6–8 min** ved 200 °C eller **190 °C** hvis panering bruner hurtigt — se farve og lad sidste minut give ekstra crunch.'
---

### Panering der sidder

Æg-laget binder mel og rasp. Hvis paneringen falder af, var overfladen for glat – prøv et ekstra drys mel efter æg, eller lad ringene “sætte” 5 minutter før airfryer.

### Kurv og batch

Luft mellem ringe er vigtigere end ved pomfritter – ellers damper undersiden. Hellere tre omgange med sprødt resultat end én fyldt kurv.

### Snack der samler menuen

Løgringe er oplagt som ekstra til burgere, hotdogs eller filmhygge – og de gør mere væsen end en ekstra skål chips, fordi sprød panering og sødt løg skaber tydelig kontrast til ketchup og dip.
