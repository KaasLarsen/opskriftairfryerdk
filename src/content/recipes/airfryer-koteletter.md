---
title: 'Airfryer koteletter – saftige koteletter med sprød stegeskorpe'
description: 'Koteletter i airfryer med tydelig tid og temperatur, så de bliver gennemstegte uden at blive tørre. Nem hverdagsopskrift.'
heroImage: '/images/recipes/airfryer-koteletter.png'
heroAlt: 'Saftige stegte koteletter med kartofler og sovs på tallerken foran en airfryer'
pubDate: 2026-03-31
updatedDate: 2026-03-31
prepMinutes: 8
cookMinutes: 16
servings: '4 portioner'
calories: 'Ca. 360 kcal pr. portion'
category: 'Hovedret'
cuisine: 'Dansk'
temperatureC: 195
keywords:
  - koteletter i airfryer
  - airfryer koteletter
  - svinekoteletter airfryer
  - koteletter tid temperatur
  - nem aftensmad
featured: true
intro: |
  Koteletter i airfryer er en af de hurtigste måder at lave svinekød på i hverdagen. Du får pæn stegeskorpe og jævn varme uden at skulle stå ved panden.

  Udfordringen med koteletter er ofte tørhed. Derfor bruger vi moderat høj varme, kort stegning og en kort hviletid før servering.

  Opskriften fungerer til både koteletter med og uden ben.
conclusion: |
  Lad koteletterne hvile et par minutter efter stegning, så saften bliver i kødet. Server med kartofler, salat eller grønt.

  Rester kan skæres i strimler og bruges i wraps eller salat dagen efter.

  Prøv med [airfryer kartofler i både](/opskrifter/airfryer-kartofler-i-baade) som klassisk tilbehør.
ingredients:
  - text: '4 svinekoteletter (ca. 700-800 g i alt)'
  - text: '1 spsk olie'
  - text: '1 1/2 tsk salt'
  - text: '1/2 tsk peber'
  - text: '1 tsk paprika'
  - text: '1/2 tsk tørret timian (valgfrit)'
instructions:
  - text: 'Dup koteletterne tørre med køkkenrulle.'
  - text: 'Vend dem med olie, salt, peber og krydderier.'
  - text: 'Forvarm airfryeren til 195 °C.'
  - text: 'Læg koteletterne i ét lag i kurven.'
  - text: 'Steg 8 minutter, vend og steg yderligere 6-8 minutter.'
  - text: 'Tjek kernen til ca. 68-70 °C og lad hvile 3 minutter.'
tips:
  - 'Tynde koteletter bliver hurtigt tørre. Tjek tidligt.'
  - 'Steg i to omgange hvis kurven er lille.'
  - 'Lidt citron ved servering frisker smagen op.'
faq:
  - question: 'Hvor længe skal koteletter have i airfryer?'
    answer: 'Typisk 14-17 minutter ved 195 °C afhængigt af tykkelse.'
  - question: 'Hvordan undgår jeg tørre koteletter?'
    answer: 'Brug ikke for lang tid, og lad kødet hvile kort efter stegning.'
  - question: 'Kan koteletter med ben laves i airfryer?'
    answer: 'Ja, men de kan kræve 1-2 minutter ekstra afhængigt af tykkelse.'
---

### Koteletter uden pandesprøjt

Koteletter i airfryer giver jævn varme omkring kødet og begrænser fedtsprøjt – praktisk på hverdage, hvor tiden er knap.

### Tykke vs. tynde koteletter

Tynde koteletter kræver kortere tid og bør tjekkes tidligt for at undgå tørhed. Tykke koteletter kan tåle lidt mere varme og tid. Derfor er det en fordel at købe koteletter med nogenlunde ens tykkelse i samme pakke.

### Marinade eller tør krydring

Tør krydring er hurtigst og giver god stegeskorpe. En kort marinade med olie, hvidløg og citron kan give mere smag, men tør kødet af før stegning, så overfladen stadig kan karamellisere pænt.
