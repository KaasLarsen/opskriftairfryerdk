---
title: 'Airfryer raznjići – kød på spyd med paprika og løg'
description: 'Raznjići i airfryer: marinerede kødtern med grøntsager på spyd. Tips til jævn stegning, saftighed og hvordan du undgår tør kant.'
pubDate: 2026-04-02
updatedDate: 2026-04-02
prepMinutes: 25
cookMinutes: 14
servings: '4 portioner (4 spyd)'
calories: 'Ca. 450 kcal pr. portion'
category: 'Hovedret'
cuisine: 'Balkansk'
temperatureC: 200
keywords:
  - raznjici airfryer
  - kødspyd airfryer
  - balkan spyd airfryer
  - svinekød spyd airfryer
  - paprika marinade airfryer
featured: false
intro: |
  Raznjići (kødspyd) er en Balkan-favorit: marinerede terninger med løg og peberfrugt, der får karakter af paprika, hvidløg og citron. Airfryer giver hurtig karamellisering — hvis spydene får plads, så varmen kan cirkulere.

  Udfordringen er ens stykker: for store bidder tager længere tid, mens små kan blive tørre. Skær derfor relativt ens og undgå at proppe for meget på hvert spyd.

  Passer perfekt sammen med [ćevapi](/opskrifter/airfryer-cevapi), en simpel bulgursalat og ajvar til at dyppe i.
conclusion: |
  Servér straks med citronbåde, groft brød og evt. grillet eller rå løg som garnish.

  Marinaden kan genbruges som idé — men kom ikke rå marinade i kontakt med færdig kød efter tilberedning.

  Til en større tallerken: tilføj [pljeskavica](/opskrifter/airfryer-pljeskavica) eller [kylling med ajvar](/opskrifter/airfryer-kylling-ajvar).
ingredients:
  - text: '600 g svinekotelet uden ben eller schnitzel, skåret i ca. 3-4 cm tern (eller kyllingelårfilet)'
  - text: '1 rød peberfrugt, i store tern'
  - text: '1 mellemstor rødløg, i kvarte/lag'
  - text: '3 spsk olivenolie'
  - text: '2 spsk citronsaft'
  - text: '1 tsk paprika (sød)'
  - text: '1/2 tsk salt'
  - text: '1/2 tsk peber'
  - text: '3 fed hvidløg, groft hakket'
  - text: '1/2 tsk tørret oregano (valgfrit)'
instructions:
  - text: 'Bland olie, citron, paprika, salt, peber, hvidløg og oregano. Vend kød og grøntsager i marinaden og lad trække 20-60 minutter på køl.'
  - text: 'Træk vekselvis kød, løg og peber på metalspyd (eller træspyd, gennemvædet i vand).'
  - text: 'Forvarm airfryeren til 200 °C.'
  - text: 'Læg spyd i kurven så der er luft mellem — brug evt. spydholder eller læg dem over en rist hvis din model tillader det.'
  - text: 'Steg 7 minutter. Vend spydene og steg 5-7 minutter mere, til kød er gennemstegt (svin typisk ca. 63-68 °C+ efter hvile — følg Fødevarestyrelsens anbefaling for svinekød) og grøntsager har kanter.'
  - text: 'Hvil 2-3 minutter før servering.'
tips:
  - 'Ens størrelse på kødtern giver bedst timing.'
  - 'Metalspyd giver ofte mere stabilitet i airfryer end træ.'
  - 'Hvis peberfrugt mørkner før kød er færdig, sænk til 190 °C og giv lidt ekstra tid.'
faq:
  - question: 'Kan man bruge kylling i stedet for svin?'
    answer: 'Ja. Brug lårfilet for saft og sigt efter ca. 74 °C i den største tern.'
  - question: 'Hvorfor bliver kødet tørt på spyd?'
    answer: 'For små tern, for lang tid, eller for tæt pakket kurv. Skær større tern og steg med luft omkring.'
  - question: 'Skal spydene vendes?'
    answer: 'Ja — de fleste airfryere giver jævnst bedst resultat med en vending midtvejs.'
  - question: 'Hvor længe tager raznjići i airfryer?'
    answer: 'Typisk 12-14 minutter ved 200 °C for mellemstore kødtern — afhænger af kødtype og model.'
---

### Balkan-marinen på få minutter

Syre fra citron, olie til varmefordeling og paprika til farve er en enkel triade, der giver tydelig “grillbar”-smag — også uden kul.

### Grøntsager som smagsdriver

Løg og peberfrugt bliver søde i kanterne og giver saft til spydene. Skær dem store nok til ikke at falde fra hinanden.

### Sikker tilberedning

Brug kernetermometer på den største kødtern og følg gældende anbefalinger for svin og fjerkræ.
