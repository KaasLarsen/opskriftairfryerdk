---
title: 'Airfryer bacon – sprødt bacon på få minutter'
description: 'Sprødt bacon i airfryer med kort tid og enkel metode. Nem guide til temperatur, stegning i hold og rengøring.'
heroImage: '/images/recipes/airfryer-bacon.png'
heroAlt: 'Sprøde stegte baconskiver på tallerken foran en airfryer'
pubDate: 2026-03-31
updatedDate: 2026-03-31
prepMinutes: 3
cookMinutes: 10
servings: '4 portioner'
calories: 'Ca. 180 kcal pr. portion'
category: 'Tilbehør'
cuisine: 'International'
temperatureC: 190
keywords:
  - bacon i airfryer
  - airfryer bacon
  - sprødt bacon airfryer
  - bacon tid temperatur
  - morgenmad airfryer
featured: true
intro: |
  Bacon i airfryer er superhurtigt og giver jævn sprødhed uden stegepande. Samtidig får du mindre stænk i køkkenet.

  Tykkelsen på baconen afgør tiden mest. Derfor bør du starte i den lave ende og justere op.

  Opskriften fungerer både til morgenmad, burger og salater.
conclusion: |
  Lad bacon hvile kort på køkkenrulle, så overskydende fedt trækkes fra og skiverne bliver ekstra sprøde.

  Brug baconen straks, eller opbevar den kort i køl til toppings.

  Kombinér med æg, salat eller kartoffelretter.
ingredients:
  - text: '200-250 g bacon i skiver'
instructions:
  - text: 'Forvarm airfryeren til 190 °C.'
  - text: 'Læg bacon i ét lag i kurven uden for meget overlap.'
  - text: 'Steg 7-10 minutter afhængigt af tykkelse.'
  - text: 'Tag bacon op og læg på køkkenrulle.'
tips:
  - 'Tynd bacon er ofte færdig efter 7-8 minutter.'
  - 'Tyk bacon kan kræve op til 11 minutter.'
  - 'Steg i hold for mest jævn sprødhed.'
faq:
  - question: 'Hvor længe skal bacon have i airfryer?'
    answer: 'Normalt 7-10 minutter ved 190 °C afhængigt af tykkelse.'
  - question: 'Skal bacon vendes i airfryer?'
    answer: 'Som regel ikke nødvendigt, men kan give mere ensartet resultat i nogle modeller.'
  - question: 'Hvordan undgår jeg røg fra bacon i airfryer?'
    answer: 'Rens kurven mellem hold og undgå for høj temperatur i starten.'
---

### Små stykker, stor effekt

Ristet bacon topper salater, æg og kartofler – i airfryer er det nemt at ramme ens sprødhed, hvis du respekterer tykkelse og tjekker sidst i forløbet.

### Tykkelse og stegning i praksis

Bacon varierer meget mellem mærker. Tynd morgenmadsbacon bliver hurtigt færdig, mens kraftige skiver kræver længere tid for at blive sprøde i midten. Start derfor altid med laveste anbefalede tid og giv 1 minut ad gangen, indtil du rammer den sprødhed du vil have.

### Brug bacon i flere retter

Sprødt bacon er perfekt som topping på salat, kartoffelretter, æg og supper. Knus baconen groft lige før servering, så den holder sin tekstur længere. Hvis du meal prepper, så opbevar den separat fra fugtige ingredienser.
